...
func (uc *UserCreate) AddChildUserIDs(ids ...int32) *UserCreate {
	uc.mutation.AddChildUserIDs(ids...)
	return uc
}

// AddChildUsers adds the "child_users" edges to the User entity.
func (uc *UserCreate) AddChildUsers(u ...*User) *UserCreate {
	ids := make([]int32, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return uc.AddChildUserIDs(ids...)
}

// AddParentUserIDs adds the "parent_users" edge to the User entity by IDs.
func (uc *UserCreate) AddParentUserIDs(ids ...int32) *UserCreate {
	uc.mutation.AddParentUserIDs(ids...)
	return uc
}

// AddParentUsers adds the "parent_users" edges to the User entity.
func (uc *UserCreate) AddParentUsers(u ...*User) *UserCreate {
	ids := make([]int32, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return uc.AddParentUserIDs(ids...)
}

// AddHeaderIDs adds the "headers" edge to the Header entity by IDs.
func (uc *UserCreate) AddHeaderIDs(ids ...int32) *UserCreate {
	uc.mutation.AddHeaderIDs(ids...)
	return uc
}

// AddHeaders adds the "headers" edges to the Header entity.
func (uc *UserCreate) AddHeaders(h ...*Header) *UserCreate {
	ids := make([]int32, len(h))
	for i := range h {
		ids[i] = h[i].ID
	}
	return uc.AddHeaderIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.ChildUsersIDs(); len(nodes) > 0 {
	...
	if nodes := uc.mutation.ParentUsersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
	...
	if nodes := uc.mutation.HeadersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,

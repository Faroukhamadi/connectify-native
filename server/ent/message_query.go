...
func (mq *MessageQuery) QueryHeader() *HeaderQuery {
	query := &HeaderQuery{config: mq.config}
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withHeader: mq.withHeader.Clone(),
...
func (mq *MessageQuery) WithHeader(opts ...func(*HeaderQuery)) *MessageQuery {
	query := &HeaderQuery{config: mq.config}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := mq.withHeader; query != nil {
		ids := make([]int32, 0, len(nodes))
		nodeids := make(map[int32][]*Message)
		for i := range nodes {
			fk := nodes[i].HeaderID
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "header_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Header = n
			}
		}
	}
